...
# 編輯 .env.local，至少填入 DATABASE_URL、RSS_URL、OPENAI_API_KEY、OPENAI_MODEL
set -a; source .env.local; set +a
pnpm db:migrate
pnpm dev
```

開啟 `http://localhost:3000`。新資料庫尚沒有 digest 是正常的；執行一次 `pnpm ingest:daily` 取得並保存當日資料。該指令需要四個變數：`DATABASE_URL`、`RSS_URL`、`OPENAI_API_KEY`、`OPENAI_MODEL`。本機開發不需要 `INTERNAL_JOB_SECRET`，除非要呼叫 `/api/internal/*` 路由。
