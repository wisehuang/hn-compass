...
function SummaryPreview({ story }: { story: PublicStory }) {
  ...
  return <div className="space-y-2 text-sm leading-6 text-zinc-700 dark:text-zinc-300">
...
export function DigestReader({ digest }: { digest: PublicDigest }) {
  return <main className="mx-auto max-w-5xl px-4 py-10 sm:px-6">
    <header className="border-b border-orange-200 pb-7 dark:border-orange-950">
      <p className="text-sm font-semibold tracking-[0.18em] text-orange-700 dark:text-orange-400">HN COMPASS · 每日精選</p>
      <h1 className="mt-2 text-4xl font-bold tracking-tight">{digest.digestDate}</h1>
      <nav className="mt-5" aria-label="每日彙整導覽"><a className="focus-ring text-sm font-semibold text-orange-700 dark:text-orange-400" href={`/daily/${digest.digestDate}`}>查看本日封存</a></nav>
    </header>
    <ol className="mt-8 space-y-6" aria-label="本日文章">
      {digest.stories.map((story) => <li key={story.id} className="border-b border-zinc-200 pb-6 dark:border-zinc-800">
        <article>
          <p className="text-sm font-semibold text-orange-700 dark:text-orange-400"><jelly-badge variant="platinum" size="small">#{story.rank}</jelly-badge> {story.sourceDomain} · {story.comments.length} 則留言</p>
...
export function StoryReader({ story }: { story: PublicStory }) {
  ...
  return <main className="mx-auto max-w-3xl px-4 py-10 sm:px-6">
    <nav aria-label="頁面導覽"><Link className="focus-ring text-sm font-semibold text-orange-700 dark:text-orange-400" href="/">← 回到最新日報</Link></nav>
    <header className="mt-7 border-b border-orange-200 pb-7 dark:border-orange-950"><p className="text-sm font-semibold text-orange-700 dark:text-orange-400"><jelly-badge variant="platinum" size="small">#{story.rank}</jelly-badge> {story.sourceDomain}</p><h1 className="mt-2 text-4xl font-bold tracking-tight">{story.title}</h1><p className="mt-4 flex flex-wrap gap-4 text-sm"><a className="focus-ring underline" href={story.articleUrl} target="_blank" rel="noreferrer">原文連結（在新分頁開啟）</a><a className="focus-ring underline" href={story.hnDiscussionUrl} target="_blank" rel="noreferrer">HN 討論（在新分頁開啟）</a></p></header>
    <section className="reader-section"><h2>文章洞見</h2>{articlePayload ? <><p>{articlePayload.tldr}</p><List items={articlePayload.keyPoints} /><p>{articlePayload.readerValue}</p><List items={articlePayload.caveats} label="注意事項" /></> : <p role="status">原文內容目前無法安全取得，因此未產生文章摘要。</p>}</section>
    <section className="reader-section"><h2>討論洞見</h2>{discussionPayload ? <><p>{discussionPayload.overview}</p><p><strong>共識：</strong>{discussionPayload.consensus ?? "證據不足或意見分歧，未下定論。"}</p><List items={discussionPayload.practicalTakeaways} label="實務建議" /><List items={discussionPayload.unresolvedQuestions} label="待釐清問題" /></> : <p role="status">尚無可用的討論摘要。</p>}</section>
    <section className="reader-section"><h2>代表性留言</h2><ol className="space-y-4">{story.comments.map((comment) => <li key={comment.hnCommentId} className="border-s-2 border-orange-300 ps-4 dark:border-orange-700"><p>{comment.bodyText}</p><p className="mt-1 text-sm text-zinc-500">{comment.author ?? "匿名"} · HN #{comment.hnCommentId}</p></li>)}</ol></section>
    <footer className="reader-section text-sm text-zinc-600 dark:text-zinc-400"><p>本頁摘要由 AI 生成，請回到原始來源核對脈絡。</p>{[article, discussion].filter(Boolean).map((summary) => <p key={summary!.kind}>{summary!.kind}：{summary!.model} · {summary!.promptVersion} · {summary!.generatedAt.toLocaleDateString("zh-TW")}</p>)}</footer>
  </main>;
}
