...
function fixture() {
  ...
  const deps: DailyDependencies = { fetchRss: async () => xml, resolveArticle: async (url) => { if (url.endsWith("/b")) throw new Error("article failure"); return { sourceUrl: url, articleFetchStatus: "SUCCESS", articleContent: "x", articleContentHash: "h", articleSummaryInput: "x" }; }, collectComments: async (id) => ({ comments: [{ hnCommentId: id * 10, parentHnCommentId: null, author: null, score: null, bodyText: "comment", position: 0, fetchedAt: new Date() }] }), createGenerator: () => ({ generateArticleFromUrl: async () => ({ payload: {}, inputHash: "a", model: "m", promptVersion: "v" }), generateDiscussion: async () => ({ payload: {}, inputHash: "d", model: "m", promptVersion: "v" }) }) };
...
describe("daily ingestion", () => {
  ...
  it("keeps an OpenAI discussion summary when Kagi article work fails", async () => {
    ...
    f.deps.createGenerator = () => ({
      generateArticleFromUrl: async () => { throw new Error("Kagi summarization failed."); },
  ...
  it("uses Kagi URL summarization even when local article extraction is unavailable", async () => {
    const f = fixture();
    f.deps.resolveArticle = async (url) => ({ sourceUrl: url, articleFetchStatus: "UNAVAILABLE", articleContent: null, articleContentHash: null, articleSummaryInput: null });
    const summarizeUrl = vi.fn(async () => ({ payload: { summary: "Kagi URL summary" }, inputHash: "article-url", model: "kagi:agnes", promptVersion: "kagi-v1" }));
    f.deps.createGenerator = () => ({
      generateArticleFromUrl: summarizeUrl,
      generateDiscussion: async () => ({ payload: { overview: "discussion" }, inputHash: "discussion", model: "gpt-test", promptVersion: "v1" }),
    });

    await runDailyIngestionWith(f.store, config, f.deps);

    expect(summarizeUrl).toHaveBeenCalledWith("https://example.test/a");
